...
def calculate_demographic_data(print_data=True):
    ...
    country = df.loc[df['salary'] == '>50K', 'native-country'].value_counts() / df['native-country'].value_counts() * 100
    highest_earning_country = country.idxmax()
    highest_earning_country_percentage = round(country[highest_earning_country], 1)

    # Identify the most popular occupation for those who earn >50K in India.
    mask_occup_india = (df['salary'] == '>50K') & (df['native-country'] == 'India')
    occupation = df[mask_occup_india].occupation.value_counts(normalize=True)
    occupation_max = occupation.max()
    top_IN_occupation = occupation[occupation == occupation_max].index.item()
